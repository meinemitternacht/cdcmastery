...
if($action){
	if($action == "add"): 
		if(isset($_POST['officeSymbolName'])):
			if($officeSymbolManager->addOfficeSymbol($_POST['officeSymbolName'])){
				$systemMessages->addMessage("Office Symbol added successfully.", "success");
				$cdcMastery->redirect("/admin/office-symbols");
			}
			else{
				$systemMessages->addMessage("There was a problem adding the Office Symbol.", "danger");
	...
	elseif($action == "edit"):
		if(isset($_POST['officeSymbolName'])):
			if($officeSymbolManager->editOfficeSymbol($officeSymbolUUID, $_POST['officeSymbolName'])){
				$systemMessages->addMessage("Office Symbol edited successfully.", "success");
				$cdcMastery->redirect("/admin/office-symbols");
			}
			else{
				$systemMessages->addMessage("There was a problem editing the Office Symbol.", "danger");
				$cdcMastery->redirect("/admin/office-symbols/edit/" . $officeSymbolUUID);
			}
		else: 
			if($officeSymbolManager->loadOfficeSymbol($officeSymbolUUID)): ?>
				<div class="container">
					<div class="row">
						<div class="8u">
							<section>
								...
								<form action="/admin/office-symbols/edit/<?php echo $officeSymbolUUID; ?>" method="POST">
									<label for="officeSymbolName">Office Symbol</label>
									<br>
									<input type="text" name="officeSymbolName" value="<?php echo $officeSymbolManager->getOfficeSymbol(); ?>">
			...
			<?php
				...
				$systemMessages->addMessage("That Office Symbol does not exist.", "danger");
				$cdcMastery->redirect("/admin/office-symbols");
			endif;
		endif; ?>
	<?php
	elseif($action == "delete"):
		if($officeSymbolManager->deleteOfficeSymbol($officeSymbolUUID)){
			$systemMessages->addMessage("Office Symbol deleted successfully.", "success");
			$cdcMastery->redirect("/admin/office-symbols");
		}
		else{
			$systemMessages->addMessage("There was a problem deleting that Office Symbol.", "danger");
			$cdcMastery->redirect("/admin/office-symbols");
		}
	endif;
}
