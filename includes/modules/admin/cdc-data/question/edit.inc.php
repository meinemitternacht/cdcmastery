...
if(!empty($_POST) && $_POST['confirmQuestionEdit'] == true){
    ...
    if(!$questionManager->saveQuestion()){
        ...
        $cdcMastery->redirect("/admin/cdc-data/".$afscManager->getUUID()."/question/".$workingChild."/edit");
    }
    else{
        /*
        ...
        $systemLog->setAction("QUESTION_EDIT");
        $systemLog->setDetail("Question UUID", $questionManager->getUUID());
        ...
        foreach($_POST['answerData'] as $answerUUID => $answerText) {
            ...
            if ($providedCorrectAnswer == $answerUUID) {
                $systemLog->setDetail("Answer Text", $answerText . " [" . $answerUUID . "] (correct)");
            } else {
                $systemLog->setDetail("Answer Text", $answerText . " [" . $answerUUID . "]");
            }
        }

        $systemLog->saveEntry();

        $systemMessages->addMessage("Question edited successfully.", "success");
        $cdcMastery->redirect("/admin/cdc-data/".$afscManager->getUUID()."/list-questions");
    }
}
?>
<div class="6u">
    <section>
        ...
        <form action="/admin/cdc-data/<?php echo $afscManager->getUUID(); ?>/question/<?php echo $workingChild; ?>/edit" method="POST">
